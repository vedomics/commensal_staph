...
# Calculate AUCs
xylo = subset(all_spf, Cohort %in% c(3, 7, 27) & Organism %in% c("PBS", "S_xylosus"))
...
# Subset the correct data
 ...
 all_bugs = subset(all_spf, Cohort %!in% c(2, 4, 8, 10, 12, 13, 15, 16, 20, 21, 23, 24, 29, 31,32,33,34,35,37) & Organism %!in% c("Sepi44_Formalin", "L_reuteri_supernatant","Sepi_EthanolKilled", "EK_Staph_hominis930", "EK_Staph_hominis931", "Sepi_HeatKilled", "Coryne", "C_tuberculostericum", "Staph_hominis931", "Sepi_NIH", "Sepi105", "Sepi44_1x") & Group %!in% c("5X_PBS", "10x_Sepi1e07", "pbs_nots", "sepi_nots", "abx_pbs","ctrl_pbs","control0","group0" ) & Day < 5)
...
# all_bugs_aucs_norm$Organism = factor(all_bugs_aucs_norm$Organism, levels= c( "PBS", "Sepi44", "S_aureus", "Staph_hominis930", "S_xylosus", "C_accolens", "L_reuteri",  "E_coli","B_subtilis"))

# aucs_labs$Organism = factor(aucs_labs$Organism, levels= c("Sepi44", "S_aureus", "Staph_hominis930", "S_xylosus", "C_accolens", "L_reuteri",  "E_coli","B_subtilis"))
...
# Plot 
  ...
  scale_x_discrete(labels = c(expression(italic("S. epidermidis")), expression(italic("S. xylosus")), expression(italic("S. aureus")), expression(italic("S. hominis")), expression(italic("C. accolens")), expression(italic("L. reuteri")), expression(italic("B. subtilis")), expression(italic("E. coli"))))+
    # scale_x_discrete(labels = c(expression(italic("S. epidermidis")), expression(italic("S. aureus")), expression(italic("S. hominis")),expression(italic("S. xylosus")), expression(italic("C. accolens")), expression(italic("L. reuteri")), expression(italic("E. coli")), expression(italic("B. subtilis"))))+
  labs(y = "Normalized TEWL AUC", x = "")+
...
# merge all sepi cohorts and killed cohorts
...
# Remove one outlier mouse that was removed after d0, and mice that were removed for being weird outliers

two_sepi_df = subset(two_sepi_df, !(Mouse == 5 & Group == "PBS" & Cohort == 12)& !(Mouse == 3 & Cohort ==17 & Group == "Sepi_HeatKilled")& !(Mouse == 4 & Cohort ==17 & Group == "Sepi_HeatKilled"))
...
### Fig 1: Healing in PBS control mice
...
ggsave(paste(supp_outfolder, "supp_fig1_a.pdf", sep="/"),  device = "pdf", dpi = 500,height = 6, width = 8 ,useDingbats = FALSE )
...
ggsave(paste(supp_outfolder, "supp_fig1_b.pdf", sep="/"),  device = "pdf", dpi = 500,height = 6, width = 8 ,useDingbats = FALSE )


```

### Fig 1: Epidermal thickness vs score
...
ggplot(subset(thicc, group %in% c("PBS", "Sepi44")), aes(x = thickness, y = score))+
    stat_smooth( method = "lm", stat = "identity", color = "grey40", alpha = 0.1, lwd = 2)+
  geom_point(aes(color = group), size = 8, alpha = 0.6)+
  scale_color_manual(values = spf_pals)+
...
ggsave(paste(supp_outfolder, "supp_fig1_d.pdf", sep="/"),  device = "pdf", dpi = 500,height = 6, width = 8 ,useDingbats = FALSE )

```


### Fig 1: Histology scores for sepi and pbs mice
...
ggsave(paste(supp_outfolder, "supp_fig1_e.pdf", sep="/"),  device = "pdf", dpi = 500,height = 6, width = 8 ,useDingbats = FALSE )


```

### Fig. 1: Endpoint CFU for S. epi colonised mice

```{r}

sepi_cfu = read.csv("../data/supplement/staph_cfu2.csv", header = TRUE)

# Add pseudocounts to our columns of interest to make plotting easier

sepi_cfu$sepi_cfu_per_g = sapply(sepi_cfu$sepi_cfu_per_g, function(x) {ifelse(x == 0, 1, x)})
sepi_cfu$sepi_cfu_per_g = as.integer(sepi_cfu$sepi_cfu_per_g)

sepi_cfu = na.omit(sepi_cfu) #otherwise plotting is insane
sepi_cfu$log = log10(sepi_cfu$sepi_cfu_per_g)

ggplot(subset(sepi_cfu, Organism  == "Sepi44"& Day == "4"), aes(x = sepi_cfu_per_g, y = skin_score))+
  geom_point(aes(color = Organism), size = 4, alpha = 0.6)+
  stat_smooth( method = "lm", color = "grey40", alpha = 0.1, lwd = 2 )+
  scale_x_log10("CFU/g",
  ...
  scale_color_manual(values = spf_pals)+
  theme_classic()+
  theme(legend.position = "bottom", aspect.ratio = 1, strip.background = element_blank())+
   stat_cor(label.y = 1, label.x = 1.5, method = "spearman")


ggplot(subset(sepi_cfu, Organism  == "Sepi44" & Day == "4"), aes(x = sepi_cfu_per_g, y = skin_tewl))+
  geom_point(aes(color = Organism), size = 4, alpha = 0.6)+
  stat_smooth( method = "lm", color = "grey40", alpha = 0.1, lwd = 2 )+
  # scale_x_log10("CFU/g",
  #       breaks = trans_breaks("log10", function(x) 10^x),
...
ggplot(subset(sepi_cfu, Organism  == "Sepi44" & Day == "4"), aes(x = sepi_cfu_per_g, y = skin_score))+
  ...
  theme_classic()+
  theme(legend.position = "bottom", aspect.ratio = 1, strip.background = element_blank())+
   stat_cor(label.y = 1, label.x = 1.5)

ggsave(paste(supp_outfolder, "supp_fig1_f.pdf", sep="/"),  device = "pdf", dpi = 500,height = 6, width = 8 ,useDingbats = FALSE )
...
### Fig 2a: S. epi dose response curve
...
sepi_dose_summary = sepi_dose_auc %>%
  group_by(Organism, Dose) %>%
  tally()
...
# Create a new class of dose that is just both low doses lumped together - this is for both plotting and stats purposes
  
mini_low_dose_df = subset(normalised_dose_auc, Dose < 10^9 & Organism == "Sepi44")
mini_low_dose_df$Dose[] <- 10 #placeholder value
mini_low_dose_df$Dose = as.integer(mini_low_dose_df$Dose)
  
normalised_dose_auc = rbind(normalised_dose_auc, mini_low_dose_df)
  
 # P vals  
df2 = normalised_dose_auc
dose_auc_labs = data.frame()
PBS_tewl = subset(df2,  Organism == "PBS")$TEWL_AUC_norm
PBS_score = subset(df2, Organism == "PBS")$Score_AUC_norm
  
for (dose in unique(subset(df2, Organism != "PBS")$Dose)){
    org_tewl_norm = subset(df2, Dose == dose)$TEWL_AUC_norm
    org_score_nrom = subset(df2, Dose == dose)$Score_AUC_norm
    tewl_p = wilcox.test(PBS_tewl, org_tewl_norm)$p.value
    score_p = wilcox.test(PBS_score, org_score_nrom)$p.value
    dose_auc_labs = rbind(dose_auc_labs, data.frame("Dose" = dose, "TEWL_p" = tewl_p, "Score_p" = score_p, "Organism" = "Sepi44"))
  } 

dose_auc_labs = dose_auc_labs %>%
   mutate(TEWL_p_sci = format(TEWL_p, scientific = TRUE, digits = 3)) %>%
   mutate(Score_p_sci = format(Score_p, scientific = TRUE, digits = 3))
  ...
  plotdf$Dose = factor(plotdf$Dose, levels = c(10,10000000, 100000000, 1000000000))
  
  # New plotting idea
  plotdf = plotdf %>%
    filter(Dose %in% c( 10,1000000000) )%>%
    mutate(CFU = case_when(Cohort == 2 & Dose == 10 ~ "1e8",
                           Cohort == 5 & Dose == 10 ~ "1e7",
                           TRUE ~ "1e9")) # for plotting purposes
  
  dose_colors = c("1e9" = "#0073C2FF", "1e8" = "#00c2b0", "1e7" = "#0012c2")
  

ggplot(plotdf,aes(x = Dose, y = TEWL_AUC_norm))+
  geom_sina(maxwidth = .7, scale = "area", seed = 1,size = 5, alpha = .5, method = "d", position = position_jitter(width = 0.3, seed = 123), aes(color=CFU, fill=CFU)) +
  geom_sina(maxwidth = .7, scale = "area", seed = 1, size = 5, shape = 1, color = "black", stroke = .8, method = "d", position = position_jitter(width = 0.3, seed = 123), aes(color=CFU, fill=CFU))+
  stat_summary(fun.y = median, geom = "errorbar", aes(ymax = ..y.., ymin = ..y.., color = "black"),
                 width = .6,size = 1, alpha = 0.9)+
  scale_x_discrete(labels= c("Low","1e9"))+
...
ggsave(paste(supp_outfolder, "supp_fig2_a_1.pdf", sep=""),  device = "pdf", dpi = 500, useDingbats = FALSE )

ggplot(plotdf,aes(x = Dose, y = Score_AUC_norm, color=CFU, fill=CFU))+
    stat_summary(fun.y = median, geom = "errorbar", aes(ymax = ..y.., ymin = ..y.., color = "black"),
                 width = .6,size = 1, alpha = 0.9)+
  geom_sina(maxwidth = .7, scale = "area", seed = 1,size = 5, alpha = .5, method = "d", position = position_jitter(width = 0.3, seed = 123), aes(color=CFU, fill=CFU)) + 
  geom_sina(maxwidth = .7, scale = "area", seed = 1, size = 5, shape = 1, color = "black", stroke = .8, method = "d", position = position_jitter(width = 0.3, seed = 123), aes(color=CFU, fill=CFU))+
  ...
  theme(panel.grid.major = element_blank(),
        ...
        panel.background = element_blank())+
   geom_hline(yintercept = 1)+
  annotate("text", label = "<0.001", x = 1, y = 3)+
  labs(y = "Normalized Score AUC", x = "")


ggsave(paste(supp_outfolder, "supp_fig2_a_2.pdf", sep=""),  device = "pdf", dpi = 500, useDingbats = FALSE )


```

### Figure 2d: Sepi applied once 
...
sepi_1x_labs = sig_labels_auc_norm(sepi1x_auc_norm)

x_pals = c("PBS" = "#868686FF" , "Sepi44_1x" = "#0073C2FF")

sepi1x_auc_norm_melt = melt(sepi1x_auc_norm, id = c("Organism", "ID", "Cohort"))
colnames(sepi1x_auc_norm_melt)[c(4,5)] = c("Metric", "Value")
sepi1x_auc_norm_melt = subset(sepi1x_auc_norm_melt, Organism != "PBS")

ggplot(sepi1x_auc_norm_melt,aes(x = Metric, y = Value, color=Organism, fill=Organism))+
    stat_summary(fun.y = median, geom = "errorbar", aes(ymax = ..y.., ymin = ..y.., color = "black"),
                 width = .6,size = 1, alpha = 0.9)+
  geom_sina(maxwidth = .7, scale = "area", seed = 1,size = 5, alpha = .5, method = "d", position = position_jitter(width = 0.3, seed = 123)) + 
  geom_sina(maxwidth = .7, scale = "area", seed = 1, size = 5, shape = 1, color = "black", stroke = .8, method = "d", position = position_jitter(width = 0.3, seed = 123))+
  ...
  scale_x_discrete(labels = c("TEWL", "Score"))+
  theme_classic()+
  theme(panel.grid.major = element_blank(),
        axis.text.x = element_text(size =18), 
        axis.text.y = element_text(size=18), axis.title = element_text(size = 20), 
        ...
        plot.margin=grid::unit(c(0,0,0,0), "pt"),
        panel.background = element_blank())+
  labs(y = "Normalized AUC", x = "")+
   geom_hline(yintercept = 1)+
  geom_text(data = sepi_1x_labs, mapping = aes(x = 1, y = 1.5, label = round(TEWL_p, 3)), color = "black")+
  geom_text(data = sepi_1x_labs, mapping = aes(x = 2, y = 2.75, label = round(Score_p, 3)), color = "black")


ggsave(paste(supp_outfolder, "supp_fig2_b.pdf", sep=""),  device = "pdf", dpi = 500, useDingbats = FALSE )
...
### Figure 2: CFU of S epi applied once
...
sepi_comp = subset(sepi_cfu, Organism %in% c("Sepi44", "Sepi44_1x"))

ggplot(sepi_comp, aes(x = Organism, y = sepi_cfu_per_g, color = Organism, Fill = Organism))+
   stat_summary(fun.y = median, geom = "errorbar", aes(ymax = ..y.., ymin = ..y.., color = "black"),
                 width = .6,size = 1, alpha = 0.9)+
  geom_sina(maxwidth = .7, scale = "area", seed = 1,size = 5, alpha = .5, method = "d", position = position_jitter(width = 0.25, seed = 123)) + 
  geom_sina(maxwidth = .7, scale = "area", seed = 1, size = 5, shape = 1, color = "black", stroke = .8, method = "d", position = position_jitter(width = 0.25, seed = 123))+
  ...
  theme(legend.position = "none", aspect.ratio = 1, strip.background = element_blank())+
  labs (x = "", y = "CFU")
  # geom_signif(comparisons = list(c("Sepi44_1x", "Sepi44")), tip_length = 0, color = "black")

ggsave(paste(supp_outfolder, "supp_fig2_d.pdf", sep=""),  device = "pdf", dpi = 500, useDingbats = FALSE )


sepi_comp2 = subset(sepi_cfu, Organism %in% c("Sepi44", "Sepi_NIH", "Sepi105"))

sepi_comp2 %>%
  group_by(Organism) %>%
  count(Organism)

ggplot(sepi_comp2, aes(x = Organism, y = sepi_cfu_per_g, color = Organism, Fill = Organism))+
   stat_summary(fun.y = median, geom = "errorbar", aes(ymax = ..y.., ymin = ..y.., color = "black"),
                 width = .6,size = 1, alpha = 0.9)+
  geom_sina(maxwidth = .7, scale = "area", seed = 1,size = 5, alpha = .5, method = "d", position = position_jitter(width = 0.25, seed = 123)) + 
  geom_sina(maxwidth = .7, scale = "area", seed = 1, size = 5, shape = 1, color = "black", stroke = .8, method = "d", position = position_jitter(width = 0.25, seed = 123))+
...
ggsave(paste(supp_outfolder, "supp_fig2_e.pdf", sep=""),  device = "pdf", dpi = 500, useDingbats = FALSE )


```
### Figure 2: ecpA expression levels


```{r}


ecpa = read.csv("../data/supplement/qpcr_ecpa.csv")
ecpa$Organism = factor(ecpa$Organism, levels = c("Sepi_NIH", "Sepi105", "Sepi44"))

ggplot(ecpa, aes(x = Organism, y = ratio_ecpa, color = Organism, Fill = Organism))+
   stat_summary(fun.y = median, geom = "errorbar", aes(ymax = ..y.., ymin = ..y.., color = "black"),
                 width = .6,size = 1, alpha = 0.9)+
  geom_sina(maxwidth = .7, scale = "area", seed = 1,size = 5, alpha = .5, method = "d", position = position_jitter(width = 0.25, seed = 123)) + 
  geom_sina(maxwidth = .7, scale = "area", seed = 1, size = 5, shape = 1, color = "black", stroke = .8, method = "d", position = position_jitter(width = 0.25, seed = 123))+
  scale_color_manual(values = spf_pals)+
  theme_classic()+
  theme(legend.position = "none", aspect.ratio = 1.25/1, strip.background = element_blank())+
  labs (x = "", y = "ecpA expressoin")

ggsave(paste(supp_outfolder, "supp_fig2_f.pdf", sep=""),  device = "pdf", dpi = 500, useDingbats = FALSE )

```

### Figure 2: Comparison of strains
...
two_sepi_df = subset(all_spf, Cohort %in% c(12, 14,27, 36,37) & Organism %in% c("PBS", "Sepi_NIH", "Sepi105") & Group %!in% c("control0", "group0", "7x_PBS-F", "5X_PBS", "5X_Sepi1e09", "10x_Sepi1e07", "Shominis930", "Sepi_SFB",  "PBS_SFB", "abx_sepi" ,"abx_pbs", "pbs_nots",  "sepi_nots"))
...
# Add in our Sepi44 data

sepi_cohorts = unique(subset(all_spf, Organism == "Sepi44" & Cohort %!in% c(4,7, 8, 10, 13, 15, 16, 20, 24,25, 28,31,32,33,34,36,37))$Cohort)

all_sepi44 = subset(all_spf, Cohort %in% sepi_cohorts & Organism %in% c("PBS", "Sepi44") & Group %!in% c("control0", "group0", "7x_PBS-F", "5X_PBS", "5X_Sepi1e09", "Shominis930", "Sepi_SFB",  "PBS_SFB", "abx_sepi" ,"abx_pbs" ) & Dose %in% c(0, 1e09))


all_sepi_strains = rbind(two_sepi_df, all_sepi44)

# AUC and normalise

sepi_aucs = auc_per_mouse(subset(all_sepi_strains, Day <5))
sepi_acus_norm = auc_norm(sepi_aucs)
sepi_aucs_labs = sig_labels_auc_norm(sepi_acus_norm)

sepi_acus_norm = subset(sepi_acus_norm, Organism %!in%  c("PBS"))

sepi_acus_norm$Organism = factor(sepi_acus_norm$Organism, levels = c("Sepi44",  "Sepi105", "Sepi_NIH" ))
sepi_aucs_labs$Organism = factor(sepi_aucs_labs$Organism, levels = c( "Sepi44",  "Sepi105", "Sepi_NIH" ))


ggplot(sepi_acus_norm, aes(x = Organism, y = TEWL_AUC_norm, color = Organism, fill = Organism))+
    geom_hline( yintercept = 1, color = "grey60", size = 0.5)+
   geom_sina(maxwidth = .7, scale = "area", seed = 1,size = 6, alpha = .5, method = "d", position = position_jitter(width = 0.2, seed = 123)) + 
  geom_sina(maxwidth = .7, scale = "area", seed = 1, size = 6, shape = 1, color = "black", stroke = .4, method = "d", position = position_jitter(width = 0.2, seed = 123))+
    stat_summary( fun.y = median, geom = "errorbar", aes(ymax = ..y.., ymin = ..y..), color = "black", width = .8,size = 1, alpha = 0.9)+
  theme_classic()+
    theme(panel.grid.major = element_blank(),
          axis.text.x = element_text(size =12, hjust = 1, vjust = 1, angle = 45), 
  ...
  # scale_y_continuous(limits = c(0.5, 2), breaks = seq(0.5,2.0,0.5))+
  labs(y = "Normalized TEWL AUC", x = "")+
  geom_signif(comparisons = list(c("Sepi44", "Sepi105"), c("Sepi105", "Sepi_NIH"), c("Sepi44", "Sepi_NIH")), step =0.1, color = "black", tip_length = 0)

ggsave(paste(supp_outfolder, "fig2_f_1.pdf", sep="/"),  device = "pdf", dpi = 500,height = 6, width = 8 ,useDingbats = FALSE )
...
ggsave(paste(supp_outfolder, "fig2_f_2.pdf", sep="/"),  device = "pdf", dpi = 500,height = 6, width = 8 ,useDingbats = FALSE )
...
### Fig 3: Day by day antibiotic treated mice
...
ggsave(paste(supp_outfolder, "supp_fig3_a.pdf", sep=""), abx_tewl,  device = "pdf", dpi = 500, useDingbats = FALSE )
...
ggsave(paste(supp_outfolder, "supp_fig3_b.pdf", sep="/"), abx_score,  device = "pdf", dpi = 500, useDingbats = FALSE )
...
### Fig.3 : Skin composition of individual mice
...
ggplot(skin_end_plot, aes(x=Sample, y= Abundance))+
  ...
  facet_wrap(Group~., scales = "free_x")+
   ...
   theme( panel.grid.major = element_blank(), 
    ...
    ggsave(paste(supp_outfolder, "supp_fig3c.pdf", sep="/"),  device = "pdf", dpi = 500,height = 6, width = 8 ,useDingbats = FALSE )
...
### Fig 3: Alpha diversity on the skin
...
ggplot(subset(skin_rich, Timepoint == "Endpoint"), aes(x = Group, y = value, fill = Group, color = Group))+
  stat_summary(fun.y = median, geom = "errorbar", aes(ymax = ..y.., ymin = ..y.., color = "black"),
                 width = .6,size = 1, alpha = 0.9)+
  geom_sina(maxwidth = .7, scale = "area", seed = 1,size = 7, alpha = .5, method = "d") + 
  geom_sina(maxwidth = .7, scale = "area", seed = 1, size = 7, shape = 1, color = "black", stroke = .8, method = "d")+
...
ggsave(paste(supp_outfolder, "supp_fig3_f.pdf", sep="/"),  device = "pdf", dpi = 500 ,useDingbats = FALSE )


```


### Fig 3: Gut Composition of indiivudal mice
...
ggplot(gut_plot, aes(x=Sample, y= Abundance))+
  ...
  facet_wrap(Group~., scales = "free_x")+
   ...
   theme( panel.grid.major = element_blank(), 
    ...
    ggsave(paste(supp_outfolder, "supp_fig3_g.pdf", sep="/"),  device = "pdf", dpi = 500,height = 6, width = 8 ,useDingbats = FALSE )
...
stat_summary(fun.y = median, geom = "errorbar", aes(ymax = ..y.., ymin = ..y.., color = "black"),
 ...
 annotate(geom = "text", label = gut_rich_pval, x = 2, y = 1.1, parse = TRUE)+
  ...
  ggsave(paste(supp_outfolder, "supp_fig3_h.pdf", sep="/"),  device = "pdf", dpi = 500,height = 6, width = 8 ,useDingbats = FALSE )


```


### Fig 3: qPCR differences in microbial load in gut and skin
...
ggplot(subset(qpcr, Timepoint == "Endpoint" & Source == "Fecal"), aes(x = Group, y = AU, fill = Group, color = Group))+
  stat_summary(fun.y = median, geom = "errorbar", aes(ymax = ..y.., ymin = ..y.., color = "black"),
                 width = .6,size = 1, alpha = 0.9)+
  geom_sina(maxwidth = .7, scale = "area", seed = 1,size = 8, alpha = .5, method = "d") + 
  geom_sina(maxwidth = .7, scale = "area", seed = 1, size = 8, shape = 1, color = "black", stroke = .8, method = "d")+
  scale_x_discrete(labels = c("Control", "Antibiotic"))+
  theme_classic()+
  scale_color_manual(values = abx_pals)+
  scale_fill_manual(values = abx_pals)+
...
ggsave(paste(supp_outfolder, "supp_fig3_i.pdf", sep="/"),  device = "pdf", dpi = 500,height = 6, width = 8 ,useDingbats = FALSE )
...
ggsave(paste(supp_outfolder, "supp_fig3_e.pdf", sep="/"),  device = "pdf", dpi = 500,height = 6, width = 8 ,useDingbats = FALSE )
...
## Fig 4: TEWL and score kinetics of S aureus
...
ggplot(subset(mrsa, Condition != "Baseline"& Day !=0), aes(x = Day, y = TEWL, color = Organism, fill = Organism))+
  ...
  geom_sina(maxwidth = .7, scale = "area", seed = 1,size = 8, alpha = .5, method = "d", position = position_dodge(width = 1)) + 
  geom_sina(maxwidth = .7, scale = "area", seed = 1, size = 8, shape = 1, color = "black", stroke = .8, method = "d", position = position_dodge(width = 1))+
    scale_color_manual(values = spf_pals)+
    scale_fill_manual(values = spf_pals)+
    theme_classic()

 
ggsave(paste(supp_outfolder, "supp_fig4_a_1.pdf", sep="/"),  device = "pdf", dpi = 500,height = 6, width = 8 ,useDingbats = FALSE )
...
ggsave(paste(supp_outfolder, "supp_fig4_a_2.pdf", sep="/"),  device = "pdf", dpi = 500,height = 6, width = 8 ,useDingbats = FALSE )
...
ggsave(paste(supp_outfolder, "supp_fig4_b_1.pdf", sep="/"),  device = "pdf", dpi = 500, useDingbats = FALSE )
...
ggsave(paste(supp_outfolder, "supp_fig4_b_2.pdf", sep="/"),  device = "pdf", dpi = 500, useDingbats = FALSE )
...
### Fig 5 : Application of S. epi to depliated, undamaged skin
...
ggplot(subset(sepinots_stats, Day >1))+
  ...
  scale_x_continuous(breaks = seq(0,4,1), expand = c(0,0))+
  scale_y_cotntinuous(breaks = seq(0,16,2))+
  labs(x = "Day", y = "TEWL")+
    theme_classic() +
  theme(panel.grid.major = element_blank(),
        axis.text.x = element_text(size =16), 
...
ggsave(paste(supp_outfolder, "supp_fig5_b_1.pdf", sep="/"),  device = "pdf", dpi = 500,height = 6, width = 8 ,useDingbats = FALSE )
...
ggsave(paste(supp_outfolder, "supp_fig5_b_2.pdf", sep="/"),  device = "pdf", dpi = 500,height = 6, width = 8 ,useDingbats = FALSE )


```

### Fig. 5: Precolonisation of healthy skin with S. epi does not improve healing outcomes to barrier damage
...
ggplot(precol_aucs_norm, aes(x = Group, y = TEWL_AUC_norm, color = Group, fill = Group))+
    stat_summary(fun.y = median, geom = "errorbar", aes(ymax = ..y.., ymin = ..y.., color = "black"),
                 width = .6,size = 1, alpha = 0.9)+
  geom_sina(maxwidth = .7, scale = "area", seed = 1,size = 8, alpha = .5, method = "d", position = position_jitter(width = 0.25, seed = 123)) + 
  geom_sina(maxwidth = .7, scale = "area", seed = 1, size = 8, shape = 1, color = "black", stroke = .8, method = "d", position = position_jitter(width = 0.25, seed = 123))+
...
ggsave(paste(supp_outfolder, "supp_fig5_d_1.pdf", sep="/"),  device = "pdf", dpi = 500,height = 6, width = 8 ,useDingbats = FALSE )
...
ggsave(paste(supp_outfolder, "supp_fig5_d_2.pdf", sep="/"),  device = "pdf", dpi = 500,height = 6, width = 8 ,useDingbats = FALSE )
...
### Fig 6: Full S. epi timecourse over 7 days
...
ggplot(subset(sepi_stats, Condition != "Baseline"))+
  ...
  scale_fill_manual(values = spf_pals, guide = "none")+
  geom_line(aes(x = Day, y = mean, color = Organism), alpha = 0.5, size = 1 )+
  geom_ribbon(aes(x = Day, y = TEWL, ymin = mean - sem, ymax = mean+sem, color = Organism, fill = Organism), alpha = 0.2, color = NA)+
...
ggsave(paste(supp_outfolder, "supp_fig6_a.pdf", sep="/"),  device = "pdf", dpi = 500,height = 6, width = 8 ,useDingbats = FALSE )

sepi_stats %>%
  filter(Condition != "Baseline" & Organism == "PBS") %>%
ggplot(.)+
  geom_point(aes(x = Day, y = mean, group=Organism,color = Organism), size = 4)+
  scale_color_manual(values = spf_pals, labels = c(expression("PBS"), expression(italic("S. epidermidis"))))+
  scale_fill_manual(values = spf_pals, guide = "none")+
...
sepi_stats %>%
  filter(Condition != "Baseline" & Organism == "PBS" & Day >1) %>%
ggplot(.)+
  geom_point(aes(x = Day, y = score_mean, group=Organism,color = Organism), size = 4)+
  scale_color_manual(values = spf_pals, labels = c(expression("PBS"), expression(italic("S. epidermidis"))))+
  scale_fill_manual(values = spf_pals, guide = "none")+
  geom_line(aes(x = Day, y = score_mean, color = Organism), alpha = 0.5, size = 1 )+
  geom_ribbon(aes(x = Day, y = TEWL, ymin = score_mean - score_sem, ymax = score_mean+score_sem, color = Organism, fill = Organism), alpha = 0.2, color = NA)+
...
ggplot(subset(sepi_stats, Condition != "Baseline" & Day > 1))+
 ...
 ggsave(paste(supp_outfolder, "supp_fig6_b.pdf", sep="/"),  device = "pdf", dpi = 500,height = 6, width = 8 ,useDingbats = FALSE )
...
ggsave(paste(supp_outfolder, "supp_fig6_c_1.pdf", sep="/"),  device = "pdf", dpi = 500,height = 6, width = 8 ,useDingbats = FALSE )
...
ggsave(paste(supp_outfolder, "supp_fig6_c_2.pdf", sep="/"),  device = "pdf", dpi = 500,height = 6, width = 8 ,useDingbats = FALSE )
...
geom_signif(comparisons = list(c("Control1", "Group1"), step = 0.1))+
 ...
 ggsave(paste(supp_outfolder, "supp_fig6_c_3.pdf", sep="/"),  device = "pdf", dpi = 500,height = 6, width = 8 ,useDingbats = FALSE )
...
ggplot(gdtcell, aes(x = Group, y = TCRg_of_DN, color = Group, fill = Group))+
  stat_summary(fun.y = median, geom = "errorbar", aes(ymax = ..y.., ymin = ..y.., color = "black"),
                 width = .6,size = 1, alpha = 0.9, position = position_dodge(width = 1))+
  geom_sina(maxwidth = .7, scale = "area", seed = 1,size = 8, alpha = .5, method = "d", position = position_jitterdodge(jitter.width = 0.25, dodge.width = 1, seed = 123)) + 
  geom_sina(maxwidth = .7, scale = "area", seed = 1, size = 8, shape = 1, color = "black", stroke = .8, method = "d", position = position_jitterdodge(jitter.width = 0.25, dodge.width = 1, seed = 123))+
  scale_fill_manual(values = color_pals)+
  scale_color_manual(values = color_pals)+
geom_signif(comparisons = list(c("Control1", "Group1"), step = 0.1), tip_length = 0)+
  ...
  ggsave(paste(supp_outfolder, "supp_fig6_c_4.pdf", sep="/"),  device = "pdf", dpi = 500,height = 6, width = 8 ,useDingbats = FALSE )
...
ggsave(paste(supp_outfolder, "supp_fig7_a.pdf", sep="/"),  device = "pdf", dpi = 500,height = 6, width = 8 ,useDingbats = FALSE )


# for other visualisation purposes
multi_elisa_melt$Protein = sapply(multi_elisa_melt$Protein, function(x) {ifelse (x == "IL12.P70", "IL.12", x)})
multi_elisa_melt_lo = subset(multi_elisa_melt, cutoff == "LO" )


multi_elisa_melt_lo$Protein = factor(multi_elisa_melt_lo$Protein , levels = c("GM.CSF", "IL.12", "IL.4", "IL.5", "IL.13",  "IFN.G", "IL.1B", "il.17", "il.23", "il.27"))

ggplot(multi_elisa_melt_lo, aes(x = Protein, y = value ))+
  geom_boxplot(aes(color=Group, fill = Group), alpha = 0.5, color = "grey20", lwd = .3)+
  theme_classic()+
    theme(panel.grid.major = element_blank(),
          strip.background = element_blank(),
          axis.text.x = element_text(size =15, hjust = 1, vjust = 1, angle = 30), 
          axis.text.y = element_text(size=12), axis.title = element_text(size = 20), 
          legend.position = "none",
          aspect.ratio = 1/1.5)+
  scale_x_discrete(labels = c("GM-CSF", "IL-12", "IL-4", "IL-5", "IL-13", "IFN-g", "IL-1b", "IL-17", "IL-23", "IL-27"))+
  # facet_wrap(cutoff~., scales = "free")+
  scale_color_manual(values = spf_pals)+
  annotate("segment", x = 7.75, xend = 8.5, y = 10, yend = 10, color = "black", linewidth = 1.25)+
  annotate("text", x = 8.15, y = 12, label = "*", size = unit(14, "pt"))+
  scale_fill_manual(values = spf_pals)+
  labs(x = "", y = "Protein ng/mL norm to total")
...
##Fig 8 : Immune volcanoes {generated in python}
## Fig 9: Flow Gating {generated in FlowJo}
...
corr_analysis_ps_sp = prune_taxa(taxa_sums(corr_analysis_ps_sp) > 0, corr_analysis_ps_sp)
corr_analysis_ps_sp = filter_taxa(corr_analysis_ps_sp, function(x){sum(x > 0.01) > 1}, prune = TRUE) # taxa that were observed at mor than 1% in at least one sample
...
for (col in c(2:ntax)){
  ...
  cortest = cor.test(skin_otus[,col], skin_otus[, "Score_AUC_norm"], method = "pearson")
  mean_abund = mean(skin_otus[,col], na.rm = T)
  cordf_sp = rbind(cordf_sp, data.frame("Taxa" = sciname, "ps_name" =colnames(skin_otus)[col], "Abundance" = mean_abund,  "Corr" = as.numeric(cortest$estimate), "P" = cortest$p.value))
}

write.csv(cordf_sp, paste(supp_outfolder, "corr_table.csv", sep = "/"))

# Lets do a quick BH correction

cordf_bh = cordf_sp[order(cordf_sp$P),]
cordf_bh$k_m_a = (seq(1,nrow(cordf_bh),1)/nrow(cordf_bh))* 0.05 #this is the critical value (rank / total obs * alpha value)
