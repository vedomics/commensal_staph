...
auc_per_mouse = function(tewl_scores, day_to_exclude_past=5) {
  ...
  auc_df_dp = tewl_scores %>%
    filter(Condition != "Baseline" & Day < day_to_exclude_past) %>%
...
df3 = df3 %>%
   ...
   mutate(Score_p_sci = format(Score_p, scientific = TRUE, digits = 3)) %>%
    mutate(TEWL_p_rounded = ifelse(TEWL_p < 0.001, "<0.001", round(TEWL_p, 3)),
         Score_p_rounded = ifelse(Score_p < 0.001, "<0.001", round(TEWL_p, 3)))
...
sepi_cohorts = unique(subset(all_spf, Organism == "Sepi44" & Cohort %!in% c(2, 4, 8, 10, 13, 15, 16, 20, 24,26,32,33,34))$Cohort)

two_sepi_df = subset(all_spf, Cohort %in% c(sepi_cohorts, 12, 14,27, 36, 17, 18, 19, 21, 23, 37) & Organism %in% c("PBS", "Sepi44", "Sepi_NIH", "Sepi105", "Sepi_HeatKilled", "Sepi_EthanolKilled") & Group %!in% c("control0", "group0", "7x_PBS-F", "5X_PBS", "5X_Sepi1e09", "10x_Sepi1e07", "Shominis930", "Sepi_SFB",  "PBS_SFB", "abx_sepi" ,"abx_pbs" ))
