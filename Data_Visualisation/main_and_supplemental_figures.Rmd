...
ggplot(subset(sepi_cfu, Organism  == "Sepi44"), aes(x = sepi_cfu_per_g, y = skin_score))+
...
### Figure 2d: Sepi applied once and S. epi strains
...
ggplot(sepi1x_auc_norm,aes(x = Organism, y = TEWL_AUC_norm, color=Organism, fill=Organism))+
  ...
  labs(y = "Normalized TEWL AUC", x = "")+
   geom_hline(yintercept = 1)+
  geom_signif(comparisons = list(c("PBS", "Sepi44_1x")), tip_length = 0)

ggsave(paste(supp_outfolder, "supp_fig2_b_1.pdf", sep=""),  device = "pdf", dpi = 500, useDingbats = FALSE )
...
ggsave(paste(supp_outfolder, "supp_fig2_b_2.pdf", sep=""),  device = "pdf", dpi = 500, useDingbats = FALSE )


```


### Figure 2: Doses across bugs
1e08 comparison of s epi, s aureus and lacto

```{r}

 low_dose = subset(all_spf, Cohort %in% c(2,3,7,9,11,14) & Organism %!in% c("Sepi21", "Coryne", "S_xylosus") & Dose != "1000000000")

low_dose_auc = auc_per_mouse(low_dose)
...
ggsave(paste(supp_outfolder, "supp_fig2_c_1.pdf", sep=""),  device = "pdf", dpi = 500, useDingbats = FALSE )
...
ggsave(paste(supp_outfolder, "supp_fig2_c_2.pdf", sep=""),  device = "pdf", dpi = 500, useDingbats = FALSE )
...
multi_elisa_melt_lo$Protein = factor(multi_elisa_melt_lo$Protein , levels = c("GM.CSF", "IL.12", "IL.4", "IL.5", "IL.13", "TGF.B", "IFN.G", "IL.1B", "il.17", "il.23", "il.27"))
...
ggplot(multi_elisa_melt_lo, aes(x = Protein, y = value ))+
  ...
  theme_classic()+
    theme(panel.grid.major = element_blank(),
          ...
          legend.position = "none",
          aspect.ratio = 1/1.5)+
  scale_x_discrete(labels = c("GM-CSF", "IL-12", "IL-4", "IL-5", "IL-13", "TGF-b", "IFN-g", "IL-1b", "IL-17", "IL-23", "IL-27"))+
  # facet_wrap(cutoff~., scales = "free")+
  scale_color_manual(values = spf_pals)+
  annotate("segment", x = 8.75, xend = 9.5, y = 10, yend = 10, color = "black", linewidth = 1.25)+
  annotate("text", x = 9.15, y = 12, label = "*", size = unit(14, "pt"))+
  scale_fill_manual(values = spf_pals)+
  labs(x = "", y = "Protein ng/mL norm to total")
